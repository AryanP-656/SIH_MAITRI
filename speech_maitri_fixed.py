"""
MAITRI Speech-to-Speech Interface with ElevenLabs (Fixed Version)
Real-time voice conversation with realistic AI voice
"""
...
class SpeechMAITRIFixed:
    """Speech-to-speech interface for MAITRI with ElevenLabs TTS (Fixed)"""
    
    def __init__(self, api_key=None, elevenlabs_api_key=None):
        """Initialize speech interface with ElevenLabs"""
        ...
        self.voice_id = "EXAVITQu4vr4xnSDxMaL"  # Bella voice (professional female)
        
        # Initialize ElevenLabs client for new API
        if ELEVENLABS_AVAILABLE and self.elevenlabs_api_key:
            if hasattr(self, 'ELEVENLABS_NEW_API') and ELEVENLABS_NEW_API:
        ...
        # Fallback TTS
        if FALLBACK_TTS_AVAILABLE and not ELEVENLABS_AVAILABLE:
            self.tts_engine = pyttsx3.init()
            self._configure_fallback_tts()
        
        # Conversation state
        self.conversation_active = False
        
        print("🎤 Speech MAITRI with ElevenLabs initialized!")
    ...
    def _configure_fallback_tts(self):
        """Configure fallback TTS settings"""
    ...
    def speak_elevenlabs_old(self, text):
        """Convert text to speech using ElevenLabs (old API)"""
        try:
            print(f"🔊 MAITRI: {text}")
            
            # Set API key using the correct method for ElevenLabs 0.2.26
    ...
    def speak_elevenlabs_new(self, text):
        """Convert text to speech using ElevenLabs (new API)"""
        try:
            print(f"🔊 MAITRI: {text}")
            
            # Generate audio with new API
    ...
    def speak_elevenlabs(self, text):
        """Convert text to speech using ElevenLabs (auto-detect API)"""
        if hasattr(self, 'ELEVENLABS_NEW_API') and ELEVENLABS_NEW_API:
            self.speak_elevenlabs_new(text)
        else:
            self.speak_elevenlabs_old(text)
    ...
    def demo_mode(self):
        ...
        print("🎬 MAITRI Speech Demo Mode with ElevenLabs")
        print("=" * 50)
...
def main():
    """Main function"""
    print("🚀 MAITRI Speech Interface with ElevenLabs (Fixed)")
    print("=" * 60)
    ...
    try:
        speech_maitri = SpeechMAITRIFixed(gemini_api_key, elevenlabs_api_key)
